...
int main (int argc, char *argv[])
{
  ...
  struct sockaddr_in Dist_Sockaddr;
  struct hostent 	*ph;
  ...
  if (getpeername(Sock_Des, (struct sockaddr *) &Dist_Sockaddr, &Taille) < 0) {
      perror("Client : erreur sur getpeername");
      exit (1);
  }
  printf("Adresse de la machine distante: %x\n", ntohs(Dist_Sockaddr.sin_addr.s_addr));
  printf("Numero du port distant: %d\n", ntohs(Dist_Sockaddr.sin_port));
  printf (" **************************************\n");
